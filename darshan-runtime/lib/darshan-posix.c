...
#define POSIX_RECORD_OPEN(__ret, __path, __mode, __tm1, __tm2) do { \
    ...
    if(__newpath != __path) free(__newpath); \
    /* dxt streamer to publish realtime read tracing information to daemon*/ \
    if(!dC.dxt_streamer_lib)\
        if(!dC.posix_enable_dxt_streamer)\
         darshan_dxt_streamer_connector_send(__rec_ref->file_rec->base_rec.id, __rec_ref->file_rec->base_rec.rank, __rec_ref->file_rec->counters[POSIX_OPENS], \
            "open", -1, -1, -1, -1, -1, __tm1, __tm2, __rec_ref->file_rec->fcounters[POSIX_F_META_TIME], "POSIX", "MET");\
} while(0)
...
#define POSIX_RECORD_READ(__ret, __fd, __pread_flag, __pread_offset, __aligned, __tm1, __tm2) do { \
    ...
    /* dxt_streamer to publish realtime read tracing information to daemon*/ \
    if(!dC.dxt_streamer_lib)\
        if(!dC.posix_enable_dxt_streamer)\
        darshan_dxt_streamer_connector_send(rec_ref->file_rec->base_rec.id, rec_ref->file_rec->base_rec.rank, rec_ref->file_rec->counters[POSIX_READS], \
            "read", this_offset, __ret, rec_ref->file_rec->counters[POSIX_MAX_BYTE_READ],rec_ref->file_rec->counters[POSIX_RW_SWITCHES], -1,  \
            __tm1, __tm2, rec_ref->file_rec->fcounters[POSIX_F_READ_TIME], "POSIX", "MOD");\
} while(0)
...
#define POSIX_RECORD_WRITE(__ret, __fd, __pwrite_flag, __pwrite_offset, __aligned, __tm1, __tm2) do { \
    ...
    /* dxt_streamer to publish realtime write tracing information to daemon*/ \
    if(!dC.dxt_streamer_lib)\
        if(!dC.posix_enable_dxt_streamer)\
            darshan_dxt_streamer_connector_send(rec_ref->file_rec->base_rec.id, rec_ref->file_rec->base_rec.rank, rec_ref->file_rec->counters[POSIX_WRITES], "write", this_offset, __ret, rec_ref->file_rec->counters[POSIX_MAX_BYTE_WRITTEN], rec_ref->file_rec->counters[POSIX_RW_SWITCHES], -1, __tm1, __tm2, rec_ref->file_rec->fcounters[POSIX_F_WRITE_TIME], "POSIX", "MOD");\
} while(0)
...
int DARSHAN_DECL(close)(int fd)
{
    ...
    if(rec_ref)
    {
        ...
        extern struct darshanConnector dC;
        if(!dC.posix_enable_dxt_streamer)
        darshan_dxt_streamer_connector_send(rec_ref->file_rec->base_rec.id, rec_ref->file_rec->base_rec.rank, -1, \
            "close", -1, -1, -1, -1, -1, tm1, tm2, rec_ref->file_rec->fcounters[POSIX_F_META_TIME], "POSIX", "MOD");
        #endif
    }
    POSIX_POST_RECORD();
